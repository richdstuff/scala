...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  override def mostRetweeted: Tweet = {
    val lMax = left.filter(x => (x.retweets > elem.retweets))
    val rMax = right.filter(x => (x.retweets > elem.retweets))
    if (lMax.isEmpty && rMax.isEmpty) elem
    else if (lMax.isEmpty) rMax.mostRetweeted
    else if (rMax.isEmpty) lMax.mostRetweeted
    else if (lMax.mostRetweeted.retweets >= rMax.mostRetweeted.retweets) lMax.mostRetweeted
    else rMax.mostRetweeted
  }
