...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences = w.toLowerCase.toList.
  	groupBy((char: Char) => char).map(x => (x._1, x._2.size)).toList.sortBy(_._1)
  ...
  lazy val dictionaryByOccurrences: Map[Occurrences, List[Word]] = dictionary
  	.map(w => (wordOccurrences(w), w)).groupBy(_._1).map(x=>(x._1, x._2.map(_._2)))

  /** Returns all the anagrams of a given word. */
  def wordAnagrams(word: Word): List[Word] = dictionaryByOccurrences.
  	find(_._1 == wordOccurrences(word)).map(_._2).getOrElse(List())
